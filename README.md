...
- 2026-07-10 再次执行默认增量抓取时，微信接口仍明确返回 `base_resp.ret=200003` / `invalid session`，说明当前 `crawl_html.py` 内的 `Cookie`/`token` 仍已失效；旧的 `data.csv` 和 `content_list.json` 已保留，未被覆盖。
- 已于 2026-07-10 基于本地缓存的 65 份 HTML 重建 `article_extracts/articles_text.json`，并补跑既有候选海报文章的定向 OCR；当前有 8 篇文章保留 OCR 文本。
...
- The incremental run on 2026-07-10 again reached the WeChat API and got `base_resp.ret=200003` / `invalid session`, which confirms the `Cookie`/`token` in `crawl_html.py` is still expired; the existing local files were kept unchanged.
- `article_extracts/articles_text.json` was rebuilt on 2026-07-10 from 65 cached HTML files, and targeted OCR was rerun for the previously identified poster-style candidate articles; 8 articles currently retain OCR text.
